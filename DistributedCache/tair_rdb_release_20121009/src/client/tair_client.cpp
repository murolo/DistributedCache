...
///////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   {
     if (is_data_server) {
       fprintf(stderr, "set config cannot use for direct mode");
     } else {
   ...
   {
     if (static_cast<int>(param.size()) < base_param_size) {
       print_help(cmd_str);
       return ;
     }
     ...
     int ret = client_helper.op_cmd_to_ds(cmd_type, &cmd_params, &infos, NULL);
   ...
   void tair_client::print_help(const char *cmd)
   {
      if (cmd == NULL || strcmp(cmd, "set_config") == 0) {
        fprintf(stderr,
            "-------------------------------------------------\n"
            "SYNOPSIS    : set_config\n"
            "DESCRIPTION: change the property value in storage engine\n"
            "===================== SubCommonad ======================\n"
            "\t\ttimed_task_time_range {x-y} : set rdb's timed task's time range\n"
            "\t\ttimed_task_clean_times {times} : set rdb's timed task's clean times\n"
            "\t\tstruct_size {list|hash|zset|set} : set rdb's structure max size\n"
            "\t\tevict_keys_each_exceed {keys} : set rdb evict keys when reach max mmemory\n"
            "\t\t*Note: when reach max memory, and in timed task time range,\n"
            "\t\t timed task each time clean keys = {keys} * {times}\n"
            );
      }
      ...
      if (cmd == NULL || strcmp(cmd, "get_config") == 0) {
        fprintf(stderr,
            "-------------------------------------------------\n"
            "SYNOPSIS    : get_config \n"
            "DESCRIPTION: get the properties in storage engine\n"
            "===================== SubCommonad ======================\n"
            "\t\ttimed_task_time_range : show rdb's timed task's time range\n"
            "\t\ttimed_task_clean_times : show rdb's timed task's clean times\n"
            "\t\tarea_detail {namespace} : show rdb's namespace detail infomation\n"
            "\t\tstruct_size : show rdb's structure max size\n"
            "\t\tevict_keys_each_exceed : show rdb evict kets when reach max memory\n"
            );
      }
      ...
      if (cmd == NULL || strcmp(cmd, "lpush") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
      ...
      if (cmd == NULL || strcmp(cmd, "rpush") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
      ...
      if (cmd == NULL || strcmp(cmd, "lindex") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
      ...
      if (cmd == NULL || strcmp(cmd, "smembers") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
      ...
      if (cmd == NULL || strcmp(cmd, "hmset") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
      ...
      if (cmd == NULL || strcmp(cmd, "hgetall") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
      ...
      if (cmd == NULL || strcmp(cmd, "hmget") == 0) {
         fprintf(stderr,
                 "------------------------------------------------\n"
...
// remove
   ...
   void tair_client::do_cmd_zadd(VSTRING &param){
      ...
      int ret = client_helper.zadd(area, key, value, score, expire, version);
   ...
   void tair_client::do_cmd_hset(VSTRING &param) {
      ...
      fprintf(stderr, "hset: key:%s field:%s value:%s area:%d\n", 
              key.get_data(), field.get_data(), value.get_data(), area);
