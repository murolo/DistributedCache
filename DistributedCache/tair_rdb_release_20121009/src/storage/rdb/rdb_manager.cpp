...
//common operator
int rdb_manager::op_cmd(ServerCmdType cmd, std::vector<std::string>& params,
    std::vector<std::string>& infos)
{
    redis_db *db = get_redis_instance();
    int ret = db->op_cmd(cmd, params, infos);
...
int rdb_manager::lazyclear(int area)
{
    ...
    log_debug("====> lazyflushdb rc: %d", ret);
    return redis_tair_code(ret);
}
...
{
    int evict_count, expired_count, put_count, get_count, hit_count, remove_count;
    ...
    for(int i = 0; i < max_area; ++i) {
      ...
      evict_count = redis_instance->evict_count(i);
      redis_instance->reset_evict_count(i);

      expired_count = redis_instance->expired_count(i);
      redis_instance->reset_expired_count(i);
      ...
      stat[i].set_remove_count(remove_count + expired_count);
      stat[i].set_hit_count(hit_count);

      log_debug(
          "area:%d dsize=usize:%ld count:%ld evect_count:%d put_count %d get_count %d remove_count %d hit_count %d",
          i,
...
void rdb_manager::get_redis_config(redisConfig* config)
{
  ...
  config->max_evict_each_time = TBSYS_CONFIG.getInt(TAIRRDB_SECTION, RDB_MAX_EVICT_EACH_TIME, -1);
  std::string time_range = TBSYS_CONFIG.getString(TAIRRDB_SECTION, RDB_TIMED_TASK_TIME_RANGE, "-1--1");
  util::time_util::get_time_range(time_range.c_str(), &config->timed_task_start, &config->timed_task_end);
  config->timed_task_clean_times = TBSYS_CONFIG.getInt(TAIRRDB_SECTION, RDB_TIMED_TASK_CLEAN_TIMES, 1);
}
