...
/*common*/
    virtual int op_cmd(ServerCmdType cmd, std::vector<std::string>& params,
        std::vector<std::string>& infos)
    {
        ...
        switch (cmd) {
          case TAIR_SERVER_CMD_SET_CONFIG:
            {
              if (params.size() == 3 && params[0] == RDB_STRUCT_SIZE) {
                int size = atoi(params[2].c_str());
                if (size <= 0) break;
                if (params[1] == RDB_LIST) {
                  log_warn("set_config list max size => %d", size);
                  context->set_list_max_size(size);
                  ret = REDIS_OK;
                } else if (params[1] == RDB_HASH) {
                  log_warn("set_config hash max size => %d", size);
                  context->set_hash_max_size(size);
                  ret = REDIS_OK;
                } else if (params[1] == RDB_ZSET) {
                  log_warn("set_config zset max size => %d", size);
                  context->set_zset_max_size(size);
                  ret = REDIS_OK;
                } else if (params[1] == RDB_SET) {
                  log_warn("set_config set max size => %d", size);
                  context->set_set_max_size(size);
                  ret = REDIS_OK;
                }
              } else if (params.size() == 2 && params[0] == RDB_EVICT_KEYS_EACH_EXCEED) {
                int size = atoi(params[1].c_str());
                if (size <= 0) break;
                log_warn("set_config set max evict keys each exceed => %d", size);
                context->set_max_evict_each_time(size);
                ret = REDIS_OK;
              } else if (params.size() == 2 && params[0] == RDB_TIMED_TASK_TIME_RANGE) {
                int start, end;
                util::time_util::get_time_range(params[1].c_str(), &start, &end);
                log_warn("set_config set timed task time range => %d-%d", start, end);
                context->set_timed_task_time_range(start, end);
                ret = REDIS_OK;
              } else if (params.size() == 2 && params[0] == RDB_TIMED_TASK_CLEAN_TIMES) {
                int times = atoi(params[1].c_str());
                if (times < 0) break;
                log_warn("set_config set timed task clean times => %d", times);
                context->set_timed_task_clean_times(times);
                ret = REDIS_OK;
              } else {
                std::stringstream ss;
                ...
                log_warn("set_config error command %s", ss.str().c_str());
              }
            }
            break;
          case TAIR_SERVER_CMD_GET_CONFIG:
            {
              if (params.size() == 0 || (params.size() == 1 && params[0] == RDB_STRUCT_SIZE)) {
                std::stringstream ss;
                ss << "ListMaxSize = "<<context->get_list_max_size()<<std::endl;
                ss << "HashMaxSize = "<<context->get_hash_max_size()<<std::endl;
                ss << "ZSetMaxSize = "<<context->get_zset_max_size()<<std::endl;
                ss << "SetMaxSize  = "<<context->get_set_max_size();
                infos.push_back(ss.str());
                ret = REDIS_OK;
              }

              if (params.size() == 0 || (params.size() == 1 && params[0] == RDB_EVICT_KEYS_EACH_EXCEED)) {
                std::stringstream ss;
                ss << "EvictKeysIfReachMaxMemory = "<<context->get_max_evict_each_time();
                infos.push_back(ss.str());
                ret = REDIS_OK;
              }

              if (params.size() == 0 || (params.size() == 1 && params[0] == RDB_TIMED_TASK_TIME_RANGE)) {
                std::stringstream ss;
                int start, end;
                context->get_timed_task_time_range(&start, &end);
                ss << "TimedTaskTimeRange = "<<start<<"-"<<end;
                infos.push_back(ss.str());
                ret = REDIS_OK;
              }

              if (params.size() == 0 || (params.size() == 1 && params[0] == RDB_TIMED_TASK_CLEAN_TIMES)) {
                std::stringstream ss;
                ss << "TimedTaskCleanTimes = "<<context->get_timed_task_clean_times();
                infos.push_back(ss.str());
                ret = REDIS_OK;
              }

              if (params.size() == 2 && params[0] == RDB_AREA_DETAIL) {
                std::stringstream ss;
                long long keys, vkeys, slots;
                size_t usedmemory, maxmemory;
                int area = atoi(params[1].c_str());
                if (area < 0 || area >= get_area_num()) break;
                int start = area * context->get_unit_num();
                int end = start + context->get_unit_num();
                ss << "DB "<<area<<":";
                for (int i = start; i < end; i++) {
                  if (context->get_area_detail_info(i,
                        &keys, &vkeys, &slots, &usedmemory, &maxmemory)) {
                    ss << std::endl << "- Unit "<< i <<": "<< keys <<" keys ("<< vkeys <<" volatile) in "
                      <<slots<<" slots HT" << ", used memory " << usedmemory<<"B, max memory " << maxmemory << "B";
                  }
                }
                infos.push_back(ss.str());
                ret = REDIS_OK;
            ...
            break;
          default:
            log_warn("op cmd %d can not process", cmd);
        }
        return ret;
    }
...
//other operator
    ...
    size_t evict_count(uint32_t area)
    {
        size_t count = 0;
        uint32_t start = area * context->get_unit_num();
        uint32_t end = start + context->get_unit_num();
        for (uint32_t i = start; i < end; i++) {
            count += context->evict_count(i);
        }
        return count;
    }

    void reset_evict_count(uint32_t area)
    {
        uint32_t start = area * context->get_unit_num();
        uint32_t end = start + context->get_unit_num();
        for (uint32_t i = start; i < end; i++) {
            context->reset_evict_count(i);
        }
    }

    size_t expired_count(uint32_t area)
    {
        size_t count = 0;
        uint32_t start = area * context->get_unit_num();
        uint32_t end = start + context->get_unit_num();
        for (uint32_t i = start; i < end; i++) {
            count += context->expired_count(i);
        }
        return count;
    }

    void reset_expired_count(uint32_t area)
    {
        uint32_t start = area * context->get_unit_num();
        uint32_t end = start + context->get_unit_num();
        for (uint32_t i = start; i < end; i++) {
            context->reset_expired_count(i);
        }
    }
