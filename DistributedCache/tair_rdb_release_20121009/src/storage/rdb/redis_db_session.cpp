...
int redis_db_session::set(MKEY, MDETA, value_item* item, EORN)
{
    ...
    DB_CONTEXT_LOCK(area);
    PROFILER_END();
    ret = get_redis_client(&client,area,getCommand(SETNXEX_COMMAND)->argc);
    if (ret != REDIS_OK) {
        return ret;
    }
...
int redis_db_session::get(MKEY, OITEMVSN)
{
    ...
    *item = new value_item;
    ...
    if(node->type == NODE_TYPE_ROBJ) {
        ...
        (*item)->data = (char*)malloc((*item)->data_len);
        memcpy((*item)->data, obj->ptr, sdslen((char*)obj->ptr));
    } else if(node->type == NODE_TYPE_LONGLONG) {
        (*item)->data = (char*)malloc(BUFFER_SIZE);
        char* buf = (*item)->data;
        size_t len = ll2string(buf, BUFFER_SIZE, node->obj.llnum);
        (*item)->data_len = len;
    }
    ...
    *item = new value_item[*item_num];
    ...
    while((node = nextValueItemNode(&it)) != NULL)
    {
        if(node->type == NODE_TYPE_ROBJ) {
            ...
            (*item)[now].data = (char*)malloc((*item)[now].data_len);
    ...
    *item = new value_item[*item_num];
    ...
    while((node = nextValueItemNode(&it)) != NULL)
    {
        if(node->type == NODE_TYPE_ROBJ) {
            ...
            (*item)[now].data = (char*)malloc(BUFFER_SIZE);
            char* buf = (*item)[now].data;
            size_t len = ll2string(buf, BUFFER_SIZE, node->obj.llnum);
            (*item)[now].data_len = len;
        }
        ++now;
    }
    freeValueItemIterator(&it);
    ...
    *ret_version = client->version;
    ...
    while((node = nextValueItemNode(&it)) != NULL)
    {
        if(node->type == NODE_TYPE_ROBJ) {
            ...
            (*item)[items_index].data = (char*)malloc((*item)[items_index].data_len);
            ...
            if (now % 2 == 0) {
                (*item)[items_index].data = (char*)malloc(BUFFER_SIZE);
    ...
    *ret_version = client->version;
    ...
    while((node = nextValueItemNode(&it)) != NULL)
    {
        if(node->type == NODE_TYPE_ROBJ) {
            ...
            (*item)[items_index].data = (char*)malloc(BUFFER_SIZE);
            char* buf = (*item)[items_index].data;
            size_t len = ll2string(buf, BUFFER_SIZE, node->obj.llnum);
            (*item)[items_index].data_len = len;
            items_index++;
        }
