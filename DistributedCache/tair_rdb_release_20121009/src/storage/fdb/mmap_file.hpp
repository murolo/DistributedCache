      ...
      public:
         ...
         ~mmap_file()
        {
          if(data) {
            ...
            munmap(data, size);
            log_debug("mmap unmapped, size is: [%zu]", size);
            data = NULL;
            size = 0;
            fd = -1;
          }
        ...
        bool map_file(bool write = false) {
          ...
          log_info("mmap file successed, maped size is: [%zu]", size);
        ...
        bool remap()
        {
          ...
          if(size == max_size) {
            log_info("already mapped max size, currSize: [%zu], maxSize: [%zu]",
                     size, max_size);
            return false;
          }
          ...
          if(new_map_data == MAP_FAILED) {
            ...
            log_info("remap success, oldSize: [%zu], newSize: [%zu]", size,
                     new_size);
          }

          log_info("mremap successed, new size: [%zu]", new_size);
          data = new_map_data;
          size = new_size;
          return true;
        }
