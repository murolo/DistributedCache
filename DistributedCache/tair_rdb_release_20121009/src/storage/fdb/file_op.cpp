  ...
  namespace storage {
    namespace fdb {
      ...
      bool file_operation::pread(void *buffer, size_t size, off_t offset)
      {
        ...
        if(is_mapped && (offset + size) <= map_file->get_size()) {
          // use mmap first
          log_debug("read data from mmap[%s], offset [%jd], size [%zu]",
                    file_name, offset, size);
          memcpy(buffer, (char *) map_file->get_data() + offset, size);
          return true;
        }

        log_debug("read from [%s], offset: [%jd], size: [%zu]", file_name,
                  offset, size);
      ...
      bool file_operation::write(void *buffer, size_t size)
      {
        ...
        log_debug("write data into with size of [%zu] at offset [%jd]", size,
                  get_position());
        ...
        if(is_mapped && (offset + size) <= map_file->get_size()) {
          log_debug("write data use mmap at offset [%jd] with size [%zu]",
                    offset, size);
      ...
      bool file_operation::pwrite(void *buffer, size_t size, off_t offset)
      {
        ...
        log_debug("sizeof(off_t): %lu, write[%s]: size [%zu] at offset [%jd]",
        ...
        if(is_mapped && (offset + size) <= map_file->get_size()) {
          // use mmap first
          log_debug("pwrite data use mmap at offset [%jd] with size [%zu]",
                    offset, size);
          memcpy((char *) map_file->get_data() + offset, buffer, size);
          return true;
        }
