...
//TODO
  ...
  {
    TBSYS_LOG(WARN, "total slab : %zu", slab_managers.size());
 ...
 *-----------------------------------------------------------------------------*/
  ...
  mdb_item *mem_cache::slab_manager::alloc_new_item(int area)
  {
    ...
    else
    {
      ...
      {
        TBSYS_LOG(ERROR,"the page id of free_head [%lu] != info->id [%d]",PAGE_ID(info->free_head),info->id);
      }
    }
    return item;
  }
 ...
 */
  ...
  void mem_cache::slab_manager::free_item(mdb_item * item)
  {
    ...
    TBSYS_LOG(DEBUG,
              "id:%lu before free:page id:%d,%p,info->free_nr:%d,info->free_head:%lu,SLAB_ID(id):%lu,",
