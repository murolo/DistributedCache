...
namespace tair
{
  ...
  class request_hmset_dup : public request_hmset {
      public:
          request_hmset_dup() : request_hmset(){
              setPCode(TAIR_REQ_HMSET_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_hmset_dup(request_hmset& packet) : request_hmset(packet) {
              setPCode(TAIR_REQ_HMSET_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_hmset::decode(input, header)) {
                  return false;
              }   
