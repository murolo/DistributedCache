...
namespace tair
{
  ...
  class request_sadd_dup : public request_sadd {
      public:
          request_sadd_dup() : request_sadd(){
              setPCode(TAIR_REQ_SADD_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_sadd_dup(request_sadd& packet) : request_sadd(packet) {
              setPCode(TAIR_REQ_SADD_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_sadd::decode(input, header)) {
                  return false;
              }   
...
class request_sadd_multi_dup : public request_sadd_multi {
      public:
          request_sadd_multi_dup() : request_sadd_multi(){
              setPCode(TAIR_REQ_SADD_MULTI_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_sadd_multi_dup(request_sadd_multi& packet) : request_sadd_multi(packet) {
              setPCode(TAIR_REQ_SADD_MULTI_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          ~request_sadd_multi_dup() {
          }   

          bool encode(tbnet::DataBuffer* output) {
              request_sadd_multi::encode(output);
              output->writeInt32(packet_id);
              output->writeInt32(bucket_id);
              return true;
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_sadd_multi::decode(input, header)) {
                  return false;
              }   
              packet_id = input->readInt32();
              bucket_id = input->readInt32();
