...
namespace tair
{
  ...
  class request_lrpush_dup : public request_lrpush {
      public:
          request_lrpush_dup(ExistOrNot en, LeftOrRight lr) : request_lrpush(en, lr){
              if (IS_NOT_EXIST) {
                  if (IS_LEFT == lr) {
                      old_pcode = TAIR_REQ_LPUSH_PACKET;
                      setPCode(TAIR_REQ_LPUSH_DUPLICATE_PACKET);
                  }
                  else {
                      old_pcode = TAIR_REQ_RPUSH_PACKET;
                      setPCode(TAIR_REQ_RPUSH_DUPLICATE_PACKET);
                  }
              }
              else {
                  if (IS_LEFT == lr) {
                      old_pcode = TAIR_REQ_LPUSHX_PACKET;
                      setPCode(TAIR_REQ_LPUSHX_DUPLICATE_PACKET);
                  }
                  else {
                      old_pcode = TAIR_REQ_RPUSH_PACKET;
                      setPCode(TAIR_REQ_RPUSHX_DUPLICATE_PACKET);
                  }
              }
              packet_id = 0;
              bucket_id = -1;
          ...
          request_lrpush_dup(int pcode) : request_lrpush(pcode){
              packet_id = 0;
              bucket_id = -1;
          }

          request_lrpush_dup(request_lrpush& packet) : request_lrpush(packet) {
              old_pcode = packet.getPCode();
              switch (old_pcode) {
          ...
          bool encode(tbnet::DataBuffer* output) {
              request_lrpush::encode(output);
              output->writeInt32(old_pcode);
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_lrpush::decode(input, header)) {
                  return false;
              }
              old_pcode = input->readInt32();
  ...
  class request_lrpush_limit_dup : public request_lrpush_limit {
      public:
          request_lrpush_limit_dup(ExistOrNot en, LeftOrRight lr) : request_lrpush_limit(en, lr){
              if (IS_LEFT == lr) {
                  old_pcode = TAIR_REQ_LPUSH_LIMIT_PACKET;
                  setPCode(TAIR_REQ_LPUSH_LIMIT_DUPLICATE_PACKET);
              }
              else {
                  old_pcode = TAIR_REQ_RPUSH_LIMIT_PACKET;
                  setPCode(TAIR_REQ_RPUSH_LIMIT_DUPLICATE_PACKET);
              }
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_lrpush_limit_dup(request_lrpush_limit& packet) : request_lrpush_limit(packet) {
              old_pcode  = packet.getPCode();
              if (TAIR_REQ_LPUSH_LIMIT_PACKET == old_pcode) {
                  setPCode(TAIR_REQ_LPUSH_LIMIT_DUPLICATE_PACKET);
              }
              if (TAIR_REQ_RPUSH_LIMIT_PACKET == old_pcode) {
                  setPCode(TAIR_REQ_RPUSH_LIMIT_DUPLICATE_PACKET);
              }
              packet_id = 0;
              bucket_id = -1; 
