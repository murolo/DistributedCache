...
class request_spop_dup : public request_spop {
      public:
          request_spop_dup() : request_spop(){
              setPCode(TAIR_REQ_SPOP_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_spop_dup(request_spop& packet) : request_spop(packet) {
              setPCode(TAIR_REQ_SPOP_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_spop::decode(input, header)) {
                  return false;
              }   
