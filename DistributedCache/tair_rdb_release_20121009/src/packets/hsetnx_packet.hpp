/*
 * (C) 2007-2010 Alibaba Group Holding Limited
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 * hset packet
 *
 * Version: $Id: hset_packet.hpp 28 2010-09-19 05:18:09Z ruohai@taobao.com $
 *
 * Authors:
 *   ruohai <ruohai@taobao.com>
 *     - initial release
 *
 */
#ifndef TAIR_PACKET_HSETNX_PACKET_H
#define TAIR_PACKET_HSETNX_PACKET_H
#include "base_packet.hpp"
namespace tair
{
  class request_hsetnx:public base_packet
  {
  public:
    request_hsetnx ()
    {
      setPCode (TAIR_REQ_HSETNX_PACKET);
      server_flag = 0;
      area = 0;
      version = 0;
      expire = 0;
    }

    request_hsetnx (request_hsetnx & packet)
    {
      setPCode (packet.getPCode ());
      server_flag = packet.server_flag;
      area = packet.area;

      version = packet.version;
      expire = packet.expire;
      key.clone (packet.key);
      field.clone (packet.field);
      value.clone (packet.value);
    }

    bool encode (tbnet::DataBuffer * output)
    {
      assert (0);
      //cpp in futrue
      return false;
    }

    bool decode (tbnet::DataBuffer * input, tbnet::PacketHeader * header)
    {

      HEADER_VERIFY;

      GETKEY_FROM_INT16 (input, version);
      GETKEY_FROM_INT32 (input, expire);
      GETKEY_FROM_DATAENTRY (input, key);
      GETKEY_FROM_DATAENTRY (input, field);
      GETKEY_FROM_DATAENTRY (input, value);

      key.set_version(version);

      return true;
    }

  public:
    uint16_t area;
    uint16_t version;
    int expire;
    data_entry key;
    data_entry field;
    data_entry value;
  };

  class response_hsetnx:public base_packet
  {
  public:

    response_hsetnx ()
    {
      config_version = 0;
      setPCode (TAIR_RESP_HSETNX_PACKET);
      code = 0;
    }

    bool encode (tbnet::DataBuffer * output)
    {
      output->writeInt32 (config_version);
      output->writeInt32 (code);

      return true;
    }

    bool decode (tbnet::DataBuffer * input, tbnet::PacketHeader * header)
    {
      assert (0);
      //cpp in future
      return false;
    }

    void set_meta (uint32_t config_version, uint32_t code)
    {
      this->config_version = config_version;
      //this->version           = version;
      this->code = code;
    }

    void set_code (int cde)
    {
      code = cde;
    }

    int get_code ()
    {
      return code;
    }

	//not used
	void set_version(uint16_t version) {}
  public:
    uint32_t config_version;
  private:
    int32_t code;
    response_hsetnx (const response_hsetnx &);
  };

  class request_hsetnx_dup : public request_hsetnx {
      public:
          request_hsetnx_dup() : request_hsetnx(){
              setPCode(TAIR_REQ_HSETNX_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_hsetnx_dup(request_hsetnx& packet) : request_hsetnx(packet) {
              setPCode(TAIR_REQ_HSETNX_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          ~request_hsetnx_dup() {
          }   

          bool encode(tbnet::DataBuffer* output) {
              request_hsetnx::encode(output);
              output->writeInt32(packet_id);
              output->writeInt32(bucket_id);
              return true;
          }   

          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_hsetnx::decode(input, header)) {
                  return false;
              }   
              packet_id = input->readInt32();
              bucket_id = input->readInt32();
              return true;
          }   
          uint32_t get_packet_id() {
                return packet_id;
            }   

            void set_packet_id(uint32_t id) {
                packet_id = id; 
            }   

        public:
            uint32_t      packet_id;
            int32_t       bucket_id;
  };  
}
#endif
