...
namespace tair
{
  ...
  class request_hdel_dup : public request_hdel {
      public:
          request_hdel_dup() : request_hdel(){
              setPCode(TAIR_REQ_HDEL_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_hdel_dup(request_hdel& packet) : request_hdel(packet) {
              setPCode(TAIR_REQ_HDEL_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_hdel::decode(input, header)) {
                  return false;
              }   
