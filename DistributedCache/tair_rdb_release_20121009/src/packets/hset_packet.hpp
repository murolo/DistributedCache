...
namespace tair
{
  ...
  class request_hset_dup : public request_hset {
      public:
          request_hset_dup() : request_hset(){
              setPCode(TAIR_REQ_HSET_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_hset_dup(request_hset& packet) : request_hset(packet) {
              setPCode(TAIR_REQ_HSET_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_hset::decode(input, header)) {
                  return false;
              }   
