...
class request_srem_dup : public request_srem {
      public:
          request_srem_dup() : request_srem(){
              setPCode(TAIR_REQ_SREM_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_srem_dup(request_srem& packet) : request_srem(packet) {
              setPCode(TAIR_REQ_SREM_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_srem::decode(input, header)) {
                  return false;
              }   
...
class request_srem_multi_dup : public request_srem_multi {
      public:
          request_srem_multi_dup() : request_srem_multi(){
              setPCode(TAIR_REQ_SREM_MULTI_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_srem_multi_dup(request_srem_multi& packet) : request_srem_multi(packet) {
              setPCode(TAIR_REQ_SREM_MULTI_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          ~request_srem_multi_dup() {
          }   

          bool encode(tbnet::DataBuffer* output) {
              request_srem_multi::encode(output);
              output->writeInt32(packet_id);
              output->writeInt32(bucket_id);
              return true;
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_srem_multi::decode(input, header)) {
                  return false;
              }   
              packet_id = input->readInt32();
              bucket_id = input->readInt32();
