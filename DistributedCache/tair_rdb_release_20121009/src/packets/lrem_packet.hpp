...
namespace tair
{
  ...
  class request_lrem_dup : public request_lrem {
      public:
          request_lrem_dup() : request_lrem(){
              setPCode(TAIR_REQ_LREM_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_lrem_dup(request_lrem& packet) : request_lrem(packet) {
              setPCode(TAIR_REQ_LREM_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_lrem::decode(input, header)) {
                  return false;
              }   
