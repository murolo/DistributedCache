...
namespace tair
{
  ...
  public:
    ...
    request_zrem(const request_zrem &packet)
    {
...
class request_zrem_dup : public request_zrem {
      public:
          request_zrem_dup() : request_zrem(){
              setPCode(TAIR_REQ_ZREM_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   

          request_zrem_dup(request_zrem& packet) : request_zrem(packet) {
              setPCode(TAIR_REQ_ZREM_DUPLICATE_PACKET);
              packet_id = 0;
              bucket_id = -1; 
          }   
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_zrem::decode(input, header)) {
                  return false;
              }   
...
#define RESPONSE_VALUES_MAXSIZE 32767
  ...
  private:
    response_zrem(const response_zrem &);
    int32_t  code;
  };
