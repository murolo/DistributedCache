...
namespace tair
{
  ...
  class request_lrpop_dup : public request_lrpop {
      public:
          request_lrpop_dup(LeftOrRight lr) : request_lrpop(lr) {
              if (lr == IS_LEFT) {
                 old_pcode = TAIR_REQ_LPOP_PACKET;
                 setPCode(TAIR_REQ_LPOP_DUPLICATE_PACKET);
              }
              else {
                 old_pcode = TAIR_REQ_RPOP_PACKET;
                 setPCode(TAIR_REQ_RPOP_DUPLICATE_PACKET);
              }
              packet_id = 0;
              bucket_id = -1;
          }

          request_lrpop_dup(int pcode) : request_lrpop(pcode) {
              packet_id = 0;
              bucket_id = -1;
          }

          request_lrpop_dup(request_lrpop& packet):request_lrpop(packet) {
              old_pcode = packet.getPCode();
              if (old_pcode == TAIR_REQ_LPOP_PACKET) {
                setPCode(TAIR_REQ_LPOP_DUPLICATE_PACKET);
              }
              if (old_pcode == TAIR_REQ_RPOP_PACKET) {
                setPCode(TAIR_REQ_RPOP_DUPLICATE_PACKET);
              }
              packet_id = 0;
              bucket_id = -1;
          ...
          bool decode(tbnet::DataBuffer* input, tbnet::PacketHeader* header) {
              if (!request_lrpop::decode(input, header)) {
                  return false;
              }
