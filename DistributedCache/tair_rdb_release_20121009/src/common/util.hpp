/*
 * (C) 2007-2010 Alibaba Group Holding Limited
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 * some commonly used util class
 *
 * Version: $Id: util.hpp 28 2010-09-19 05:18:09Z ruohai@taobao.com $
 *
 * Authors:
 *   ruohai <ruohai@taobao.com>
 *     - initial release
 *
 */
#ifndef TAIR_MHASH_HPP
#define TAIR_MHASH_HPP
#include <sys/time.h>
#include <sys/types.h>
#include <dirent.h>
#include <ctype.h>
#include <stdint.h>
#include <stdlib.h>
#include <stdio.h>
#include "hash.hpp"

#include <stddef.h>
#include <unistd.h>
#include <assert.h>
#include <errno.h>
#include <string.h>
#include <string>
#include "log.hpp"

namespace tair
{
   namespace util
   {
      class hash_util
      {
      public:
         static unsigned int mhash1(char *key, int size)
          {
            return mur_mur_hash2(key, size, 134217689);
         }

         static unsigned int mhash2(char *key, int size)
          {
            return mur_mur_hash2(key, size, 97);
         }

      };

      class time_util
      {
      public:
         static bool get_time_range(const char* time_range, int* start, int* end) {
            /*
             * start < 0 or end < 0 or start > 24 or end > 24 => return false, -1, -1
             * */
            int index = 0;
            int number = 0;
            if (time_range[index] == '-') {
              *start = -1;
              *end = -1;
              return false;
            }

            while(time_range[index] >= '0' && time_range[index] <= '9') {
              number = number * 10 + (time_range[index] - '0');
              index++;
            }
            if (number > 24) {
              *start = -1;
              *end = -1;
              return false;
            }
            if (time_range[index] != '-') {
              *start = -1;
              *end = -1;
              return false;
            }
            *start = number;
            index++;
            int x = index;
            number = 0;
            while (time_range[index] >= '0' && time_range[index] <= '9') {
              number = number * 10 + (time_range[index] - '0');
              index++;
            }
            if (index == x) {
              *start = -1;
              *end = -1;
              return false;
            }
            if (number > 24 || time_range[index] != '\0') {
              *start = -1;
              *end = -1;
              return false;
            }
            *end = number;
            return true;
         }
      };

      class string_util
      {
      public:

         static char *conv_show_string(char *str, int size, char *ret = NULL, int msize = 0)
          {
            int index = 0;
            if (ret == NULL) {
               msize = size*3+5;
               ret = (char*) malloc(msize);
            }
            unsigned char *p = (unsigned char *)str;
            while (size-->0 && index<msize-4) {
               index += sprintf(ret+index, "\\%02X", *p);
               p ++;
            }
            ret[index] = '\0';
            return ret;
         }

         static void conv_raw_string(const char *str, char *result, int *size)
         {
            int index = 0;
            const unsigned char *p = (const unsigned char *)str;
            while(*p && index<(*size)-1) {
               if (*p == '\\') {
                  int c1 = *(p+1);
                  int c2 = *(p+2);
                  if (c1==0 || c2==0) break;
                  if (isupper(c1)) c1 = tolower(c1);
                  int value = (c1 >= '0' && c1 <= '9' ? c1 - '0' : c1 - 'a' + 10) * 16;
                  if (isupper(c2)) c2 = tolower(c2);
                  value += c2 >= '0' && c2 <= '9' ? c2 - '0' : c2 - 'a' + 10;
                  result[index++] = (char)(value&0xff);
                  p += 2;
               } else {
                  result[index++] = *p;
               }
               p ++;
            }
            *size = index;
         }

         static unsigned int mur_mur_hash(const void *key, int len)
         {
            return mur_mur_hash2(key, len, 97);
         }
      };

      class local_server_ip {
      public:
         static uint64_t ip;
      };

   }

}
#endif
