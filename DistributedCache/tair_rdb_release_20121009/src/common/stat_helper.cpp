...
namespace tair {
   ...
   {
       log_debug("stat_helper::reset ==========> 1111111111111");
      if (compressed_data != NULL && sent == false) return;

       log_debug("stat_helper::reset ==========> 222222222222");
      ...
      for (int i=0; i<STAT_LENGTH; i++) {
         ...
         log_debug("area:%d evict_count:%d get_count:%d put_count:%d remove_count:%d hit_count:%d item_count:%d \
                 data_size:%d use_size:%d",
                 i, cs->evict_count(), cs->get_count(), cs->put_count(), cs->remove_count(), cs->hit_count(),
                 cs->item_count(), cs->data_size(), cs->use_size());
      }

      // compress
      do_compress();
      sent = false;
       log_debug("stat_helper::reset ==========> 3333333333333333");
   }
