...
#define METHOD_HVALS    2
   ...
   public:
     ...
     // bool initialize(tbnet::Transport *transport, tbnet::DefaultPacketStreamer *streamer);
      ...
      /* string operator */
      int put(int area, data_entry & key, data_entry & value, int expire_time, base_packet *request,int heart_vesion);
      int putnx(int area, data_entry & key, data_entry & value, int expire_time, base_packet* request, int heart_version);
      ...
      int lrpush(int area, data_entry& key, vector<data_entry *> &value,
          ...
          uint16_t version, int expire_time, base_packet* request, int heart_version);

      int lrpop (int area, data_entry & key, int popnum, vector<data_entry *>& value,
              int pcode, uint16_t version, int expire_time, base_packet* request, int heart_version);
      ...
      int lrem (int area, data_entry & key, int32_t count, data_entry & val,
  	      long long *retnum, uint16_t version, int expire_time, base_packet* request, int heart_version);

      int lrange (int area, data_entry & key, int32_t start, int32_t end,
  		vector<data_entry *>& values);

      int lset (int area, data_entry & key, int32_t index, data_entry & val,
          uint16_t version, int expire_time, base_packet* request, int heart_version);
      ...
      int hmset (int area, data_entry& key, vector<data_entry *>& field_val,
              int* retvalue, uint16_t version, int expire_time, base_packet* request, int heart_version);

      int hset (int area, data_entry & key, data_entry & field,
  	      data_entry & val, uint16_t version, int expire_time, base_packet* request, int heart_version);
	  ...
	  int zremrangebyrank (int area, data_entry & key, int start, int end,
			  uint16_t version, int expire_time, long long *remnum, base_packet* request, int heart_version);
      ...
      void do_duplicate(data_entry& key, data_entry& mkey, base_packet* request , int heart_version); 

      uint32_t get_bucket_number(data_entry &key);

      dup_sync_sender_manager* get_duplicator() {
          return duplicator;
      }

      bool NeedCopy(int server_flag);
   ...
   private:
      ...
      dup_sync_sender_manager *duplicator;
