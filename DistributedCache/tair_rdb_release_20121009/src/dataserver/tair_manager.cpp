...
#define STAT_KEY_VERIFY  do {                                           \
   ...
   int tair_manager::lset (int area, data_entry & key, int32_t index, data_entry & value,
       uint16_t version, int expire_time, base_packet* request, int heart_version)
   {
     STAT_KEY_VERIFY;
     ...
     int rc = storage_mgr->lset (area, mkey, index, value, version_care, version, expire_time);
     key.data_meta = mkey.data_meta;
 ...
 /* zset */
   ...
   int tair_manager::zadd (int area, data_entry & key, double score,
 			  data_entry & value, uint16_t version, int expire_time, base_packet* request, int heart_version)
   {
     STAT_KEY_VERIFY;
   ...
   {
      log_debug("receive dump request, size: %zu", dump_meta_infos.size());
   ...
   bool tair_manager::NeedCopy(int server_flag) {
       int op_flag = get_op_flag(server_flag);
