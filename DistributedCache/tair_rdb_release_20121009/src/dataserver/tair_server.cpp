...
namespace tair {
   ...
   bool tair_server::handlePacketQueue(tbnet::Packet *apacket, void *args)
   {
      ...
      switch (pcode) {
		  ...
		  case TAIR_REQ_PUT_PACKET:
		  {
              ...
              if (TAIR_RETURN_SUCCESS == ret) {
                  log_error("put success");
                  ret = tair_mgr->get_duplicator()->duplicate_data(npacket, heartbeat.get_client_version());
              }
              */
			  break;
		  }
      ...
      {
        request_lset *npacket = (request_lset *) packet;
        ret = req_processor->process (npacket, send_return);
        log_debug ("end lset, prepare to send return packet");
        break;
      }
      ...
      case TAIR_REQ_HKEYS_PACKET:
      {
         ...
         */
         ...
         //duplicate
          case TAIR_REQ_PUT_DUPLICATE_PACKET:
          {
              request_put_dup* npacket = (request_put_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_PUTNX_DUPLICATE_PACKET:
          {
              request_putnx_dup *npacket = (request_putnx_dup *)packet;
              ret = req_processor->process (npacket, send_return);
              break;
          }
          case TAIR_REQ_REMOVE_DUPLICATE_PACKET:
          {
              request_remove_dup *npacket = (request_remove_dup *)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
		  case TAIR_REQ_LPUSH_DUPLICATE_PACKET:
		  case TAIR_REQ_RPUSH_DUPLICATE_PACKET:
		  case TAIR_REQ_LPUSHX_DUPLICATE_PACKET:
		  case TAIR_REQ_RPUSHX_DUPLICATE_PACKET:
          {
              request_lrpush_dup *npacket = (request_lrpush_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_LTRIM_DUPLICATE_PACKET:
          {
              request_ltrim_dup *npacket = (request_ltrim_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_LREM_DUPLICATE_PACKET:
          {
              request_lrem_dup *npacket = (request_lrem_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_LSET_DUPLICATE_PACKET:
          {
              request_lset_dup *npacket = (request_lset_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_HMSET_DUPLICATE_PACKET:
          {
              request_hmset_dup *npacket = (request_hmset_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_HSET_DUPLICATE_PACKET:
          {
              request_hset_dup *npacket = (request_hset_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_HSETNX_DUPLICATE_PACKET:
          {
              request_hsetnx_dup *npacket = (request_hsetnx_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_HDEL_DUPLICATE_PACKET:
          {
              request_hdel_dup *npacket = (request_hdel_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_SPOP_DUPLICATE_PACKET:
          {
              request_spop_dup *npacket = (request_spop_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_SREM_DUPLICATE_PACKET:
          {
              request_srem_dup *npacket = (request_srem_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_SADD_DUPLICATE_PACKET:
          {
              request_sadd_dup *npacket = (request_sadd_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_SADD_MULTI_DUPLICATE_PACKET:
          {
              request_sadd_multi_dup *npacket = (request_sadd_multi_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_SREM_MULTI_DUPLICATE_PACKET:
          {
              request_srem_multi_dup *npacket = (request_srem_multi_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_ZADD_DUPLICATE_PACKET:
          {
              request_zadd_dup *npacket = (request_zadd_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_ZREM_DUPLICATE_PACKET:
          {
              request_zrem_dup *npacket = (request_zrem_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_ZREMRANGEBYRANK_DUPLICATE_PACKET:
          {
              request_zremrangebyrank_dup *npacket = (request_zremrangebyrank_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
          case TAIR_REQ_ZREMRANGEBYSCORE_DUPLICATE_PACKET:
          {
              request_zremrangebyscore_dup *npacket = (request_zremrangebyscore_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
		  case TAIR_REQ_LPUSH_LIMIT_DUPLICATE_PACKET:
		  case TAIR_REQ_RPUSH_LIMIT_DUPLICATE_PACKET:
		  case TAIR_REQ_LPUSHX_LIMIT_DUPLICATE_PACKET:
		  case TAIR_REQ_RPUSHX_LIMIT_DUPLICATE_PACKET:
          {
              request_lrpush_limit_dup *npacket = (request_lrpush_limit_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
		  case TAIR_REQ_LPOP_DUPLICATE_PACKET:
		  case TAIR_REQ_RPOP_DUPLICATE_PACKET:
          {
              request_lrpop_dup *npacket = (request_lrpop_dup*)packet;
              ret = req_processor->process(npacket, send_return);
              break;
          }
         default:
...
{   
    ...
    char path[ 256 ];
    sprintf( path, "/home/s/var/proc/agile_dataserver.%d.pid", pid );
    ...
    log_debug( "[maintain][pid=%d][ppid=%d]write %s to %s file OK\n",
