...
namespace tair{
   ...
   void bucket_waiting_queue::push(request_duplicate_packet& p_packet, uint64_t server_id)
   {
      ...
      p_packet->set_packet_id( atomic_add_return(1, &packet_id_creater) );
   ...
   bool bucket_waiting_queue::pop(uint32_t packet_id, uint64_t server_id)
   {
      ...
      if (p_packet->get_packet_id() != packet_id) return false;
      it->second.pop();
      last_send_time[server_id] = 0;
      return true;
   }
   ...
   bool bucket_waiting_queue::send(int64_t now)
   {
      ...
      for (it = packets_queue.begin(); it != packets_queue.end(); ++it) {
         ...
         request_duplicate_packet packet = queue.front();
         log_debug("will send packet pid = %d", packet->get_packet_id());
         PROFILER_START("do duplicate");
         PROFILER_BEGIN("send duplicate packet");
         if (psm->conn_mgr->sendPacket(it->first, packet.get(), NULL, NULL, true) == false) {
   ...
   int duplicate_sender_manager::duplicate_data(const vector<uint64_t>& des_server_ids, base_packet *request, int version){
       return 0;
   }
