...
int staticNumericData(char** buffer) {
  ...
  *buffer = (char*)malloc(sizeof(char) * 32);
  ...
  snprintf(*buffer, 32, "%d", rand() & 0x7fffffff);
...
data_entry* data_entry_numeric_rand(bool isrand) {
    char* buffer = NULL;
    if (isrand) {
      if (!randNumericData(&buffer)) return 0;
    } else {
      if (!staticNumericData(&buffer)) return 0;
    }

    data_entry* entry = new data_entry(buffer, strlen(buffer), false);
    return entry;
}
...
DVECT* vector_double_rand(int size, bool isrand ) {
  DVECT* dvect = new DVECT();
  if (dvect == NULL) {
    return NULL;
  }
...
VDE* vector_data_entry_numeric_rand(int size, bool isrand) {
  VDE* vde = new VDE();
  if (vde == NULL) return NULL;
  for (int i = 0; i < size; i++) {
    data_entry* entry = data_entry_numeric_rand(isrand);
    if (entry == NULL) {
      vector_data_entry_free(vde, 1);
      return NULL;
    }
...
MDE* map_data_entry_numeric_rand(int size, bool isrand) {
  MDE* mde = new map<data_entry*, data_entry*>();
  if (mde == NULL) return NULL;

  for (int i = 0; i < size; i++) {
    data_entry* key = data_entry_numeric_rand(isrand);
    if (key == NULL) {
      map_data_entry_free(mde, 1);
      return NULL;
    }
    data_entry* value = data_entry_numeric_rand(isrand);
    if (value == NULL) {
      map_data_entry_free(mde, 1);
      data_entry_free(key, 1);
      return NULL;
    }
    mde->insert(std::make_pair(key, value));
  }
...
MMDE* mmap_data_entry_numeric_rand(int size, int ssize, bool isrand) {
  MMDE* mmde = new MMDE();
  if (mmde == NULL) return NULL;
  ...
  for (int i = 0; i < size; i++) {
    ...
    MDE* mde = map_data_entry_numeric_rand(ssize, isrand);
    if (mde == NULL) {
      data_entry_free(key, 1);
      mmap_data_entry_free(mmde, 1);
      return NULL;
    ...
    mmde->insert(std::make_pair(key, mde));
  }
...
MVDE* mapv_data_entry_numeric_rand(int size, int ssize, bool isrand) {
  MVDE* mvde = new MVDE();
  if (mvde == NULL) return NULL;
  ...
  for (int i = 0; i < size; i++) {
    ...
    VDE* vde = vector_data_entry_numeric_rand(isrand);
    if (vde == NULL) {
      data_entry_free(key, 1);
      mapv_data_entry_free(mvde, 1);
      return NULL;
    ...
    mvde->insert(std::make_pair(key, vde));
  }
  return mvde;
}
